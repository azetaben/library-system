...
export const getBrowserArtifactLabel = () =>
    sanitizeReportFileName(process.env.BROWSER?.trim() || 'unknown-browser');

export const getBrowserQualifiedArtifactName = (baseName: string) =>
    `${getBrowserArtifactLabel()}-${sanitizeReportFileName(baseName)}`;

export const getScreenshotPath = (scenarioName: string) =>
    path.join(screenshotsDir, `${getBrowserQualifiedArtifactName(scenarioName)}.png`);

export const getTracePath = (scenarioName: string) =>
    path.join(tracesDir, `${getBrowserQualifiedArtifactName(scenarioName)}-trace.zip`);
