...
import type {LoginCredentials, NegativeLoginExample} from '../types/login.types.ts';
import {logger} from '../../runtime/logger.ts';

export class LoginDataFactory {
    static invalidUser(): LoginCredentials {
        logger.debug('Generating invalid-user login data');
        return {
    ...
    static empty(): LoginCredentials {
        logger.debug('Generating empty login data');
        return {
            username: '',
            password: '',
        };
    ...
    static sqlInjection(validPassword = 'admin'): LoginCredentials {
        logger.debug('Generating SQL-injection-style login data');
        return {
            username: `' OR '1'='1`,
    ...
    static leadingTrailingSpaces(): LoginCredentials {
        logger.debug('Generating login data with leading and trailing spaces');
        return {
            username: ' admin ',
            password: ' admin ',
        };
    ...
    static negativeExamples(validUsername = 'admin'): NegativeLoginExample[] {
        logger.debug('Generating negative login examples');
        return [
            {
                key: 'unknown-user',
    ...
    static negativeExampleByKey(
        ...
        logger.debug(`Resolving negative login example "${key}"`);
