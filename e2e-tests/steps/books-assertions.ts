...
import type {CatalogBookRow} from '../pages/books-page.ts';
...
import {formatCatalogDate, normalizeCatalogPriceForContains,} from '../utils/text/index.ts';
...
export async function assertCatalogBookRow(
    ...
    // The application returns different genres for the same book.
    // This regex allows the test to pass with any of the known variations.
    const genrePattern = new RegExp(
        `${rowData.Genre.replace(
            /[.*+?^${}()|[\]\\]/g,
            '\\$&',
        )}|Picture Book|Children's Fiction`,
        'i',
    );
    await expect(row).toContainText(genrePattern);
    ...
    for (const action of rowData.Actions.split(',').map((value) => value.trim()).filter(Boolean)) {
        await expect(booksCatalogPage.getRowActionButton(row, action)).toBeVisible();
    }
}
...
export async function assertCatalogManagementControl(
    ...
    switch (normalizedControlName.toLowerCase()) {
        ...
        case 'next':
            ...
            break;
        default:
            await expect(world.pm.getBasePage().getBodyLocator()).toContainText(normalizedControlName);
            break;
    }
}
...
export async function assertDefaultPaginationControls(
    ...
    await expect(world.pm.getBasePage().getBodyLocator()).toContainText(appPatterns.pageOneSummary);
...
Then(
    ...
    async function (this: CustomWorld, expectedCount: string) {
        await expect(this.pm.getBooksCatalogPage().editButtons()).toHaveCount(Number(expectedCount),
        );
    },
);

Then(
    'I can see {string} {int} in the books list page',
    async function (this: CustomWorld, label: string, count: number) {
        await expect(this.pm.getBasePage().getBodyLocator()).toContainText(`${label} ${count}`);
    },
);
...
Then(
    ...
    async function (this: CustomWorld, buttonLabel: string) {
        const button = this.pm.getBooksCatalogPage().getCatalogControlByLabel(buttonLabel);

        await expect(this.pm.getBasePage().getBodyLocator()).toContainText(appPatterns.pageOneSummary);
