...
import type {EditBookFormData, EditBookPage} from '../pages/edit-book-page.ts';
...
export async function assertEditBookButtonClickable(
    ...
    const knownButtons: Record<string, () => ReturnType<typeof editBookPage.saveChangesButton.isVisible>> = {
        'save changes': () => editBookPage.saveChangesButton.isVisible(),
    };

    const normalizedName = buttonName.trim().toLowerCase();

    if (normalizedName in knownButtons) {
        await expect(editBookPage.saveChangesButton).toBeVisible();
        await expect(editBookPage.saveChangesButton).toBeEnabled();
        return;
    }

    // Generic fallback: locate any button/anchor with matching text
    const genericButton = editBookPage.buttonByExactLabel(buttonName);

    await expect(genericButton).toBeVisible();
    await expect(genericButton).toBeEnabled();
}
...
export async function assertEditBookValidationMessage(
    ...
    const bodyText = (await editBookPage.getBodyText()).trim();
    if (ValidationMessageMapper.isEquivalentMessage(bodyText, expectedMessage)) {
        logger.debug('Validation message found in body using equivalent pattern');
        return;
    }
    ...
    const stillVisible = (await editBookPage.getBodyText()).toLowerCase();
